...
def _low_rank_decomposition(weight, reduced_rank=32):
    ...
    if weight.device == torch.device('cpu'):
        print("weight is on CPU, moving to GPU")
    ...
    if CPU_FLAG:
        ...
        print("weight is on CPU after SVD")
        print()
...
class manual_float8_matmul_with_args_in_float8(torch.autograd.Function):
    ...
    def backward(ctx, grad_output_fp8):
        ...
        out_A_fp8 = hp_tensor_and_scale_to_float8(out_A, out_A_scale, torch.float8_e4m3fn, linear_mm_config=ctx.linear_mm_config, gemm_input_role=GemmInputRole.WEIGHT, axiswise_dim=None)
...
class Float8Linear_falqon(torch.nn.Linear):
    ...
    def apply_delta_weight(self):
        ...
        merged_weight = torch.cat([fp8_W_orig_t, fp8_A.to(B_dtype).t()], dim=0).clamp(min=torch.finfo(orig_datatype).min, max=torch.finfo(orig_datatype).max).to(orig_datatype).contiguous().t()
    ...
    def from_float(
        ...
        with torch.no_grad():
            ...
            fp8_merged_weight = new_mod.cast_weight_to_float8_t(merged_weight, new_mod.is_amax_initialized, fp8_merged_weight_scale)
