...
def set_module_tensor_to_device(
    ...
    with torch.no_grad():
        ...
        if is_buffer:
            ...
            elif param_cls.__name__ in ["Float8Tensor"]:
                new_value = torch.nn.Parameter(new_value).to(device)
            else:
...
def compute_module_sizes(
    ...
    for name, tensor in module_list:
        if isinstance(tensor, Float8Tensor):
            size = tensor.numel() * dtype_byte_size(tensor._data.dtype)
        elif special_dtypes is not None and name in special_dtypes:
